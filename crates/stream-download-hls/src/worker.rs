...
use stream_download::storage::{DynamicLength, SegmentedLength};

use crate::cache::keys::{CacheKeyGenerator, HlsCacheKey};
...
pub struct HlsStreamWorker {
    data_sender: mpsc::Sender<bytes::Bytes>,
    ...
    // Cache provider for read-before-fetch caching probes (segments, playlists/keys in manager).
    ...
    /// Deterministic master hash used for cache keys.
    master_hash: String,
}
...
impl HlsStreamWorker {
    ...
    fn maybe_update_default_stream_key(
        ...
        if *last_variant_id != Some(desc.variant_id) {
            ...
            // TODO: Update for new cache architecture
    ...
    fn stream_key_for_desc(&self, desc: &crate::manager::SegmentDescriptor) -> HlsCacheKey {
        // For persistent caching with deterministic file-tree layout, the stream key MUST be:
        // `"<master_hash>/<variant_id>"`.
        // TODO: Update for new cache architecture
        HlsCacheKey::new(format!("{}/{}", self.master_hash, desc.variant_id.0))
        // TODO: Update for new cache architecture
        // ResourceKey(format!("{}/{}", self.master_hash, desc.variant_id.0).into())
        HlsCacheKey::new(format!("{}/{}", self.master_hash, desc.variant_id.0))
    }
    ...
    fn cached_segment_key(
        &self,
        stream_key: &HlsCacheKey,
        filename_hint: &Option<Arc<str>>,
    ) -> Option<HlsCacheKey> {
        // We address segments by a HlsCacheKey that is the relative path from storage_root:
        ...
        filename_hint.as_deref().map(|base| {
            ...
            HlsCacheKey::new(format!("{}/{}", stream_key.as_str(), base))
        })
    }

    /// Best-effort cache probe.
    /// Returns `Some(len)` on HIT, `None` on MISS / error / disabled.
    fn probe_cached_segment_len(&self, seg_key: &Option<HlsCacheKey>) -> Option<u64> {
        ...
        //     Ok(Some(len)) => {
        //         tracing::trace!("segment cache: HIT key='{}' len={}", seg_key.0, len);
        //         Some(len)
        //     }
        //     Ok(None) => {
        ...
        //     }
        //     Err(e) => {
        //         tracing::trace!("segment cache: error key='{}' error='{:?}'", seg_key.0, e);
        //         None
        //     }
        // }
        None
    ...
    async fn emit_cached_segment_as_chunk(
        ...
        //     }))
        //     .await
        //     .map_err(|_| HlsError::ChannelClosed)?;
    ...
    pub async fn new(
        ...
        // Read-before-fetch caching for playlists/keys uses the cache provider.
